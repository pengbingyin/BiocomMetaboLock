...
for row, time_point in enumerate(time_points):
    for col, tube in enumerate(desired_tubes):
        ...
        # Bandwidth with higher minimum for smoother early time points
        std_fitc = np.std(fitc_data)
        bw_fitc = 10 / std_fitc if std_fitc > 0 else 1.0
        bw_fitc = max(bw_fitc, 0.12)   # ← Increased minimum bandwidth for smoothness
        ...
        ax.plot(x, y_fitc, color='green', linewidth=1.1)   # slightly thicker line
