...
input_dir = os.path.join("data", "ME612Dreference")
output_dir = os.path.join("output", "ME612Dreference")
...
for root, dirs, files in os.walk(input_dir):
    for filename in files:
        ...
        try:
            ...
            # ===== Robust FITC-A voltage extraction =====
            TMD = 350.0

            # We know from the diagnostic that FITC-A is parameter 4 in these files
            # and the voltage is stored as $P4V
            if '$P4V' in meta:
                try:
                    TMD = float(meta['$P4V'])
                    print(f"      FITC-A Voltage (TMD) = {TMD}  (from $P4V)")
                except (ValueError, TypeError):
                    print("      Could not convert $P4V, using default 350")
            else:
                # Fallback: try other common positions
                for key in ['$P3V', '$P5V', '$P2V']:
                    if key in meta:
                        try:
                            TMD = float(meta[key])
                            print(f"      FITC-A Voltage (TMD) = {TMD}  (from {key} - fallback)")
                            break
                        except:
                            pass
                else:
                    print("      FITC-A Voltage not found, using default 350")
